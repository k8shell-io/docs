...
* **Resource provisioning.** The Provisioner verifies users' identity, retrieves custom blueprints and assembles final workspace blueprint.
* **Authorization.** The Provisioner evaluates authorization policies for workspace provisioning and applies any obligations that may alter the final assembled blueprint. See [Workspace domain](../authz/domain-workspace.md) for more details.
* **Provisioning.** The Provisioner uses the Kubernetes API to create the workspace resource set. Depending on the deployment model, it either creates a [standalone pod](standalone-pod.md) or [injects the workspace](workload-injection.md) into an existing workload by patching its pod template.
