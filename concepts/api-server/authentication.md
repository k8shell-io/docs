...
* **OAuth web flow.** The API Server forwards the authentication request to the [Identity service](/concepts/identity), which performs the OAuth web flow with the chosen provider — handling the provider redirect, token exchange, and user profile retrieval.
