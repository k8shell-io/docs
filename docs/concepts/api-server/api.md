...
**`GET /login`** — Initiates the [OAuth web flow](/concepts/identity/user-management#web-flow) for the selected identity provider. Redirects the browser to the provider's authorization endpoint. 
...
**`GET /credentials/:name`** — Returns the [credential helper](/concepts/identity/credential-helpers) record for `git`, `registry`, or `kubernetes`.
...
**`GET /:username/credentials/:name`** — Returns a [credential helper](/concepts/identity/credential-helpers) for a specific service for the user.
...
**`POST /`** — Provisions a new workspace. The request body specifies the blueprint and user-string parameters. The API Server forwards the request to the [Provisioner](/concepts/provisioner) and returns a provision job ID for progress tracking.
...
**`GET /`** — Returns the list of [apps](/concepts/workspace/apps) registered for the workspace. 
